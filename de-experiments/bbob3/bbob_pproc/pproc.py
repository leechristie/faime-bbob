...
class DataSet():
    ...
    def splitByTrials(self, whichdata=None):
        ...
        for instanceid, idx in list(dictinstance.items()):
...
class DataSetList(list):
    ...
    def run_length_distributions(self, dimension, target_values,
        ...
        for k, v in list(rld_dict.items()):
        ...
        if flatten_output_dict and len(rld_dict) == 1:
            return list(rld_dict.values())[0], left_envelope
...
def dictAlgByDim(dictAlg):
    ...
    for alg, dsList in list(dictAlg.items()):
...
def dictAlgByDim2(dictAlg, remove_empty=False):
    ...
    if remove_empty:
        raise NotImplementedError
        for dim, ds_dict in list(res.items()):
            for alg, ds_dict2 in list(ds_dict.items()):
                if not len(ds_dict2):
                    pass
            if not len(ds_dict):
                pass
