...
function_IDs = ()
function_IDs = list(range(1,200))  # sep ros high mul mulw == 1, 6, 10, 15, 20, 101, 107, 122, 
...
def main(dictAlg, order=None, outputdir='.', info='default',
    ...
    xbest2009 = []
    maxevalsbest2009 = []
    for f, dictAlgperFunc in list(dictFunc.items()):
    ...
    if len(dictFunc) == 1:
        plt.title(' '.join((str(list(dictFunc.keys())[0]),
                  genericsettings.current_testbed.short_names[list(dictFunc.keys())[0]])))
    a = plt.gca()
